...
class Eventbrite_Manager {
	...
	public function request( $endpoint, $params = array(), $id = false, $force = false ) {
        ...
        //write_log($endpoint . ' it is true before call');
	...
	/**
	 * Create new events on EventBrite.
	 *
	 * @access public
	 *
	 * @param array $params Parameters to be passed during the API call.
	 ...
	 */
	public function post_event($course_id, $force = false ) {
		// Prepare meta data for post the course to eventbrite.
		$params = $this->map_course_keys_to_ebapi($course_id);
		// if event has already on eventbrite, update the field value instead of create a new event;
		// if eventbrite_event_id is not existing in course field,
		// (means the this is the first time to create the event)
		// post a new event to eventbrite
		if ( ! empty(get_post_meta( $course_id, 'eventbrite_event_id'))){
			$event_id = get_post_meta($course_id, 'eventbrite_event_id', true);
			$results = $this->request( 'update_event', $params, intval($event_id), $force=true );
		}
		else{
			//endpoint 'post_new_event' corresponding to post events on eventbrite
			write_log('This is for create a new event on eventbrite!');
			$results = $this->request( 'post_new_event', $params, false, $force );

			// Update course URI and eventbrite event ID on wp course
			add_post_meta($course_id, 'eventbrite_event_id', $results->id, false);
			add_post_meta($course_id, 'eventbrite_event_url', $results->url, false);
		}

		// When the course status change from 'draft' to 'public', make the course publish
        // 1. create eventbrite tickets
        // 2. change event status to publish
		if($results->status == 'draft' && $_POST['post_status'] =='publish')
        {
            // step 1: create eventbrite ticket class
            $ticket_params = array();
            $ticket_params['ticket_class.name'] = 'GTF Course Ticket';
            $ticket_params['ticket_class.free'] = true;
            $ticket_params['ticket_class.quantity_total'] =$params['event.capacity'];
            $ticket_class = $this->request('create_ticket_class', $ticket_params, intval($event_id), $force=true);

            //step 2: publish the event
            $this->request('publish_event', array(), intval($event_id), $force=true);
        }
	...
	protected function map_event_keys( $api_event ) {
        ...
        $event['quantity_sold'] = (isset( $api_event->ticket_classes[0] ->quantity_sold)) ? $api_event->ticket_classes[0] ->quantity_sold : '';

		return (object) $event;
	}

	/**
	 * Convert the custom post type Course into properties to create eventbrite events.
	 *
	 * @access protected
	 *
	 * @param object  a single event from the API results.
	 * @return object Event with Eventbrite_Event keys.
	 */
	protected function map_course_keys_to_ebapi( $course_id ) {
		$api_event = array();

		$api_event['event.name.html'] = (isset($_POST['post_title'])) ? $_POST['post_title'] : '';
		$api_event['event.description.html'] = (isset($_POST['post_content'])) ? $_POST['post_content'] : '';

		if(isset( $_POST['acf']['field_gtf_course_start_date'])){
			$start_time = $_POST['acf']['field_gtf_course_start_date'];
			$api_event['event.start.utc'] = $this->helper_format_date_time($start_time);
			$api_event['event.end.utc'] = $this->helper_format_date_time($start_time);
		}
		else{
			$api_event['event.start.utc'] = date("Y-m-d") . 'T09:00:00Z';
			$api_event['event.end.utc'] = date("Y-m-d") . 'T09:00:00Z';
		}

		$api_event['event.start.timezone'] = 'America/New_York';
		$api_event['event.end.timezone'] = 'America/New_York';
		$api_event['event.listed'] = false;
		$api_event['event.currency'] = 'USD';
		$api_event['event.capacity'] = (isset($_POST['course_capacity'])) ? $_POST['course_capacity'] : 0;
		$api_event['event.show_remaining'] = true;

		return $api_event;
	}

	public function helper_format_date_time($date){
		write_log($date);
		$d = date_create_from_format('Ymd', $date);
		return $d->format('Y-m-d') . 'T08:00:00Z';
	}

...
class Eventbrite_API extends Keyring_Service_Eventbrite {
	...
	private function define_endpoints() {
		...
		$this->set_endpoint( 'post_new_event', self::API_BASE . 'events/', 'POST');
		$this->set_endpoint( 'update_event', self::API_BASE . 'events/', 'POST');
		$this->set_endpoint( 'create_ticket_class', self::API_BASE . 'events/' , 'POST');
        $this->set_endpoint( 'publish_event', self::API_BASE . 'events/', 'POST');
	}
	...
	public static function call( $endpoint, $query_params = array(), $object_id = null ) {
		...
		if ($endpoint == 'create_ticket_class'){
            $endpoint_url = $endpoint_url . 'ticket_classes/';
		}

        write_log($endpoint);
        // POST /events/:id/publish/
        if($endpoint == 'publish_event'){
            $endpoint_url = $endpoint_url . 'publish/';
        }
        write_log($endpoint_url);
